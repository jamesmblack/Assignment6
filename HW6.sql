...
SET GLOBAL mysqlx_connect_timeout = 1200;
...
-- NOTE: Adjusted procedure each time for changing records. 
...
SELECT 
    ...
    CREATE INDEX idx ON accounts (branch_name, account_type);
...
CREATE PROCEDURE measure_query_time(IN query_string TEXT)
BEGIN
...
Call measure_query_time('SELECT count(*) FROM accounts WHERE branch_name = "Mianus" AND balance = 50000');
